...
> A user who likes to do puzzles and quizzes on their very long commute to work. They want to have the option to play with lots of questions to keep them entertained. They would like the game to easily function from the web on a mobile device without needing to scroll a lot or spend too much time figuring out how the website works.
---
## Competitor Review
...
- To give options for different genres of quiz
...
| To give options for different genres of quiz                      | 3          | 2                       | Blue   |
...
![Scatter graph of the Objectives, Importance and Viability/Feasibility table](/assets/images/readme/objectives-scatter-graph.PNG "Objectives, Importance and Viability/Feasibility table")
...
I created a Rules page in HTML and a Rules Modal using JavaScript. Both are a simplistic div with green borders, and six easy to read rules of how to play the game.
...
Each button has a hover quality to make the colour of the div a bit darker, demonstrated below on the Rules div.
...
#### Imagery
The Play and Rules buttons on the home page also have great images. As mentioned previously, I specifically chose the background colour of the divs to match the background colour in the two images used for the hover property. This creates the effect of the pencils or question marks appearing and disappearing when the user hover over or off.
...
#### 4K Screens
On largest screens up to 2560px, the background image is consistently filling the whole page. The main div with a white background adjusts according to screen size easily as well. Rules and Play buttons height is dependent on the view height. The gap between each Navbar element also adjusts easily, and the same is for each Footer social element. The text at the top of the page and footer are both centrally aligned and adjust to size.

On the Game and Setup pages, the four divs in the centre are well-spaced and have good gaps between them. On the Score page, the Answers section fills the centre gap and the grid inside holding the answers adjusts its margins to the size of the screen. 
...
The Setup and Game divs are a good square shape, and consistent in size with one another to make the flow from the Setup to the Game page easy for the user. 

#### Laptop
At Laptop size, the Play and Rules buttons are closer to the size of other elements on the page. Text sizes have adjusted again but have done so consistently. The message at the top of the Home page has changed to 20px, but the Play and Rules titles are still at 30px to use some of the space in their divs.

On the Setup and Game pages, the four divs are almost square and are a good size to hold each of the answers. 
...
On the home page, the Play and Rules divs are now stacked one on top of the other, rather than side by side. On the Game and Setup pages, the four divs change into one column of four that are well-spaced. The Rules Page and Modal adjust to the size of the page and the text wraps well to the next line.
...
To fix this, I asked JavaScript to create a paragraph element for each answer in the coloured boxes, and then added styling to align these to the centre. This then created a problem with the score page, as the question number, selected answer, and tick or cross were now all on separate lines, as below.
...
#### Nav bar
I added a home icon and the Quiz Whizz logo to the top of the home box, but struggled to get these to fit in line with each other. Originally, I made the home icon fixed and gave it top and left properties. However, when I wanted to add the Rules modal, I couldn't keep the logo centred as easily. I added the Rules button first and styled it with CSS.
![Score Page Bug fix](/assets/images/readme/bugs/nav-bar-issue-1.PNG "Score Page Bug fix")

Then I gave it a top property of 50px, a left property of 150px and `position: absolute;`. This put the Rules button in the right place.
...
I fixed this issue by changing my approach. Instead of randomly selecting each answer from the array, I shuffled the four strings inside the array by putting them in Alphabetical order using the sort() function.
...
The noticed that although JavaScript was unable to display the answer correctly, HTML could. Therefore, the best fix I found was to create a div on the game page with the CSS property `display: none;` and asked JavaScript to display the correct answer in this div. I then changed the if statement checking whether the user's answer was correct to check whether it matched the innerHTML of the invisible div, instead of checking against the value the API had produced.

#### Social Link Circles
I wanted to add my social links into circles so the circles would be coloured, and then have a hover quality of a white background and just their border coloured. However, I was having issues with the circles showing as tall ovals and being inconsistent to one another in size.
...
---
## Manual Testing
Manual testing  was completed separately in the [test.md](test.md) file
...
- I build the project on [Gitpod](https://gitpod.io/)
...
- I used the boilerplate code for Jest Testing in the [Code Institute](https://codeinstitute.net/) lessons to set up my game testing
...
- I also used [Convertio](https://convertio.co/) and [Pixelied PNG to WEBP Converter](https://pixelied.com/convert/png-converter/png-to-webp) to convert the images to a .WEBP file.
